...
{
  FILE *output_file = fopen("D:\\SEM5\\SP\\ese\\A6_literal\\assign7output.txt", "w"); // Changed file name here
  if (output_file == NULL)
  {
...
int main()
{
  FILE *file = fopen("D:\\SEM5\\SP\\ese\\A6_literal\\assign7input.txt", "r");
  if (file == NULL)
  {
  ...
  fclose(file);
  printf("Lexical analysis completed. Output written to assign7input.txt\n");
  return 0;
}